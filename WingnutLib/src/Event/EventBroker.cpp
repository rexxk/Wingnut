...
namespace Wingnut
{
	...
	void EventBroker::ProcessEvent(Ref<Event> event)
	{
		switch (event->Type())
		{
			case EventType::KeyPressed: EventCallback<KeyPressedEvent>::Execute(std::dynamic_pointer_cast<KeyPressedEvent>(event)); return;
			case EventType::KeyReleased: EventCallback<KeyReleasedEvent>::Execute(std::dynamic_pointer_cast<KeyReleasedEvent>(event)); return;
			case EventType::KeyTyped: EventCallback<KeyTypedEvent>::Execute(std::dynamic_pointer_cast<KeyTypedEvent>(event)); return;
			...
			case EventType::MaterialSelected: EventCallback<MaterialSelectedEvent>::Execute(std::dynamic_pointer_cast<MaterialSelectedEvent>(event)); return;
