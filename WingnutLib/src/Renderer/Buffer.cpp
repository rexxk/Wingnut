...
namespace Wingnut
{
	...
	void CopyBuffer(Ref<Device> device, VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize bufferSize)
	{
		...
		commandBufferAllocateInfo.commandPool = Renderer::GetRendererData().TransferCommandPool->GetCommandPool();
		...
		vkFreeCommandBuffers(device->GetDevice(), Renderer::GetRendererData().TransferCommandPool->GetCommandPool(), 1, &commandBuffer);
	}
